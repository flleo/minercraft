...
# Built material
bte = Entity(model='block', texture=blockTex)
...
for i in range(numSubCubes):
    bud = Entity(model='cube', texture=blockTex)
    ...
    if subDic.get('x' + str(x) + 'z' + str(z)) != 'i':
        ...
        if currentCube == numSubCubes:
            ...
            if currentSubset == numSubsets:
                megasets.append(Entity(texture=blockTex, model='cube'))
...
for i in range(shellWidth * shellWidth):
    bud = Entity(model='block', collider='box')
    bud.visible = False
    shellies.append(bud)
